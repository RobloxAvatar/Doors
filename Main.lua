...
do
	local Window = library:AddWindow("Doors", {
		main_color = Color3.fromRGB(13, 105, 172),
    ...
    local engine = loadstring(game:HttpGet("https://raw.githubusercontent.com/RobloxAvatar/Doors/main/Engine.lua"))()

	do
		MainTab:AddLabel("this is the most stable version!")
        CharacterTab:AddLabel("this is the most stable version!")
		EspTab:AddLabel("this is the most stable version!")

        getgenv().mobspawnnotify = false
        getgenv().avoid = false

		local switch = MainTab:AddSwitch("Mob Spawn Notify", function(bool)
	        getgenv().mobspawnnotify = bool
        end)
        switch:Set(false)

        local switch = MainTab:AddSwitch("Avoid Rush/Ambush", function(bool)
	        getgenv().avoid = bool
        end)
        switch:Set(false)
		...
		local switch = EspTab:AddSwitch("Esp", function(bool)
	        getgenv().esp = bool
        end)
        switch:Set(false)
		...
		local function ApplyDoorsChams(inst)
			wait()
			local Cham = Instance.new("Highlight")
			Cham.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
			Cham.FillColor = Color3.fromRGB(0, 0, 255)
			Cham.FillTransparency = 0.5
		...
		spawn(function()
			while task.wait() do
				for i,v in pairs(workspace:GetDescendants()) do
					if v.Name == "Door" and v:IsA("MeshPart") and getgenv().esp == false then
		...
		spawn(function()
			while task.wait() do
				if game:GetService("Workspace").CurrentRooms:FindFirstChild("50") and getgenv().esp == false then
					for i,v in pairs(game:GetService("Workspace").CurrentRooms["50"].Assets:GetDescendants()) do
		...
		spawn(function()
			while task.wait() do
				if game:GetService("Workspace").CurrentRooms:FindFirstChild("50") and getgenv().esp == false then
					for i,v in pairs(game:GetService("Workspace").CurrentRooms["50"].FigureSetup:GetDescendants()) do
		...
		spawn(function()
			while task.wait() do
				for i,v in pairs(workspace:GetDescendants()) do
					if v.Name == "Door" and v:IsA("MeshPart") and getgenv().esp == true then
						if not v:FindFirstChild("Cracked esp") then
							esp.Text = "Door"
							gui:Clone().Parent = v
						end
					end
        ...
        MainTab:AddButton("Auto Complete(Seek)", function()
	        local LatestRoom = game:GetService("ReplicatedStorage").GameData.LatestRoom
            if workspace:FindFirstChild("SeekMoving") then
                if LatestRoom.Value < 50 then
                    for i = 1,7 do
                        game.Players.LocalPlayer.Character:PivotTo(CFrame.new(game:GetService("Workspace").CurrentRooms[tostring(LatestRoom.Value)].Door.Door.Position))
                        wait(0.5)
                    end
        ...
        local slider = CharacterTab:AddSlider("Speed", function(ws)
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            pcall(function()
                if game.Players.LocalPlayer.Character.Humanoid.MoveDirection.Magnitude > 0 then
                    game.Players.LocalPlayer.Character:TranslateBy(game.Players.LocalPlayer.Character.Humanoid.MoveDirection * getgenv().walkspeed/50)
                end
            end)
        end)

        workspace.ChildAdded:Connect(function(inst)
            if inst.Name == "RushMoving" and getgenv().avoid == true then
                engine.MakeNotification("Avoiding Rush. Please wait.", 5)
                ...
                if inst.Name == "AmbushMoving" and getgenv().avoid == true then
                    engine.MakeNotification("Avoiding Rush. Please wait.", 5)
                    local OldPos = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
                    local con = game:GetService("RunService").Heartbeat:Connect(function()
                        game.Players.LocalPlayer.Character:MoveTo(OldPos + Vector3.new(0,20,0))
        ...
        workspace.ChildAdded:Connect(function(inst)
            if inst.Name == "RushMoving" and getgenv().mobspawnnotify == true and getgenv().avoid == false then
        ...
        LatestRoom.Changed:Connect(function()
            n = ChaseStart.Value - LatestRoom.Value
            if n < 3 and n ~= 0 and n ~= -1 and not string.find(n, "-") and getgenv().mobspawnnotify == true then
                engine.MakeNotification("Event in " .. "'" .. tostring(n) .. "'" .. " rooms.", 3)
            end
        end)
        MainTab:Show()
